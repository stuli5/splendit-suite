...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const r1 = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 560px; color: #0f2e2a;">
          <h2 style="color: #00a87a; margin-bottom: 4px;">New ${typeLabel} uploaded</h2>
          <p style="color: #7ab8ae; margin-top: 0; font-size: 13px;">${new Date().toLocaleString('cs-CZ')}</p>

          <table style="width:100%; border-collapse: collapse; margin: 20px 0; font-size: 14px;">
            <tr style="background: #f0faf8;"><td style="padding: 8px 12px; font-weight: bold;">Contractor</td><td style="padding: 8px 12px;">${candidateName}</td></tr>
            <tr>                             <td style="padding: 8px 12px; font-weight: bold;">Email</td>      <td style="padding: 8px 12px;">${candidateEmail}</td></tr>
            <tr style="background: #f0faf8;"><td style="padding: 8px 12px; font-weight: bold;">Type</td>      <td style="padding: 8px 12px;">${typeLabel}</td></tr>
            <tr>                             <td style="padding: 8px 12px; font-weight: bold;">Month</td>     <td style="padding: 8px 12px;">${month}</td></tr>
            <tr style="background: #f0faf8;"><td style="padding: 8px 12px; font-weight: bold;">Amount</td>   <td style="padding: 8px 12px;">${amountLabel}</td></tr>
            <tr>                             <td style="padding: 8px 12px; font-weight: bold;">File</td>      <td style="padding: 8px 12px;">${fileName}</td></tr>
          </table>

          ${fileUrl ? `<a href="${fileUrl}" style="display:inline-block; padding: 10px 20px; background: #00a87a; color: white; text-decoration: none; border-radius: 6px; font-weight: bold;">Download file</a>` : ''}
    ...
    const r2 = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 560px; color: #0f2e2a;">
          <h2 style="color: #00a87a;">Your ${typeLabel.toLowerCase()} was received</h2>
          <p>Hi ${candidateName},</p>
          <p>We received your <strong>${typeLabel.toLowerCase()}</strong> for <strong>${month}</strong>${amount ? ` (${amountLabel})` : ''}.</p>
